...
jQuery(document).ready(function ($) {
    ...
    if ($.cookie("onetop_str_item_id")) {
        ...
        if (product_id) {
            $.ajax({
                url: '/Cart/command',
                type: 'post',
                data: { "co": product_id, "color": color, "size": size },
                dataType: 'json',
    ...
    $(".add-to-cart").click(function(){
        ...
        $("#zoom1 img:last").clone().prependTo("#zoom1").css({'position':'absolute'}).animate({opacity:0.4},100).animate(
        {opacity:0, marginLeft:gotoX, marginTop:gotoY, width:newImageWidth, height:newImageHeight},1200,function(){
            $(this).remove();
            debugger
            ...
            if(product_id){
                ...
                $.ajax({
                    ...
                    type:'post',
                    data: { "co": product_id, "type": 0, "color": color, "size": size, "sl": count },
                    dataType:'json',
                    success:function(data){
                        $('.showCartTop').html(data.success);
                        $('.numCart').text(data.html);
    ...
    $('a.btnAddCart').click(function () {
        ...
        var co = $.cookie("onetop_str_item_id");
        if (count != '') {
            $.post('/Cart/UpdateTopCart', { "co": product_id, "type": 1, "sl": count },
